...
const list: SkillDef[] = [
  ...
  // Sabreur — ARBRE à deux branches offensives + deux passifs. Racines libres, feuilles verrouillées
  // (niveau + prérequis) → on ne peut pas tout monter au max, il faut choisir sa voie.
  // Branche « Lame » (frappe lourde) : taillade → attaque-chargée → { plongeon, lame ultime, folie enragée }.
  ...
  // BUFF UNIQUE du sabreur : Folie enragée — aura ROUGE SANG + terreur. Seul buff actif de la classe.
  ...
  // Branche « Vif » (rotation / lancer) : estoc-rapide → tourbillon → lancer d'épée.
  ...
  // Passifs (jamais équipés, actifs dès appris) — voie de survie du guerrier.
  { id: 'double-saut', name: 'Double saut', description: 'Passif : débloque un second saut en plein vol. Le saut aérien CONSOMME de l\'énergie et démarre modeste (faible hauteur) ; monte son niveau pour qu\'il rattrape peu à peu la hauteur du premier saut.', classId: 'swordsman', kind: 'passive', multiplier: 1, cooldownMs: 1, range: 0, minLevel: 4 },
  { id: 'regeneration', name: 'Régénération', description: 'Passif : le sabreur récupère lentement des PV au fil du temps, tant qu\'il n\'a pas été touché depuis quelques secondes. +2 PV/s par rang.', classId: 'swordsman', kind: 'passive', multiplier: 1, cooldownMs: 1, range: 0, minLevel: 2, passive: { hpRegenPerSec: 2 } },
  // Épée enflammée : conservée au registre (saves / emprunts) mais HORS de l'arbre actif (un seul buff par classe).
  ...
  // Branche « Feu » : boule de feu → éclairs → mur de flamme → pluie de météores ; embranchement buff.
  ...
  { id: 'eclair', name: 'Éclairs foudroyants', description: 'De gros éclairs bleus zigzaguent devant le mage : courte portée, mais dégâts frontaux dévastateurs.', classId: 'mage', kind: 'lightning', multiplier: 2.8, cooldownMs: 6500, range: 250, minLevel: 10, requires: 'boule-de-feu' },
  ...
  { id: 'pluie-de-meteores', name: 'Pluie de météores', description: 'Vise une zone : une pluie de météores ardents s\'abat du ciel et explose au sol dans un fracas de flammes.', classId: 'mage', kind: 'zone', multiplier: 2.6, cooldownMs: 17000, range: 150, meteors: 7, minLevel: 20, requires: 'mur-de-flamme' },
  // BUFF UNIQUE du mage : Fureur arcanique — aura VIOLETTE d'arcanes crépitante (visuel distinct).
  { id: 'fureur-arcanique', name: 'Fureur arcanique', description: 'Le mage s\'entoure d\'une aura d\'arcanes violette et crépitante : sa puissance magique décuple un long moment.', classId: 'mage', kind: 'buff', multiplier: 1, cooldownMs: 14000, range: 0, buff: { atkMult: 1.6, durationMs: 8000 }, minLevel: 16, requires: 'eclair' },
  // Branche « Givre / soutien » : nova de givre → soin.
  ...
  // Passifs (jamais équipés) : maîtrise arcanique → vitalité magique.
  ...
  // Branche « Tir direct » : flèche perçante → { tir chargé → pluie de flèches, flèche enflammée → flèche explosive, buff }.
  ...
  { id: 'tir-charge', name: 'Tir chargé', description: 'Une flèche longuement bandée, lourde et fulgurante, qui frappe la première cible de plein fouet.', classId: 'archer', kind: 'projectile', multiplier: 2.8, cooldownMs: 10000, range: 650, minLevel: 14, requires: 'fleche-percante' },
  { id: 'pluie-de-fleches', name: 'Pluie de flèches', description: 'Vise une zone : une nuée de flèches s\'abat du ciel et crible tout le secteur, encore et encore.', classId: 'archer', kind: 'zone', multiplier: 1.5, cooldownMs: 12000, range: 150, rain: 46, minLevel: 18, requires: 'tir-charge' },
  { id: 'fleche-enflammee', name: 'Flèche enflammée', description: 'Une flèche embrasée qui frappe la première cible et la laisse brûler dans la durée.', classId: 'archer', kind: 'projectile', multiplier: 2.2, cooldownMs: 6000, range: 660, burn: true, minLevel: 10, requires: 'fleche-percante' },
  { id: 'fleche-explosive', name: 'Flèche explosive', description: 'Un tir en cloche à tête explosive : il retombe et détone au sol dans une gerbe de feu.', classId: 'archer', kind: 'projectile', multiplier: 2.4, cooldownMs: 7000, range: 560, arc: true, explode: true, explodeRadius: 120, minLevel: 12, requires: 'fleche-enflammee' },
  // Flèche AUTOGUIDÉE : traverse murs et terrain, enchaîne d'ennemi en ennemi (l'ennemi le plus PROCHE
  // pas encore touché, dans une portée ~largeur d'écran) ; le nombre de cibles = le RANG (1→1 … 5→5).
  ...
  // BUFF UNIQUE de l'archer : Œil du lynx — aura VERTE d'agilité (visuel distinct).
  ...
  // Branche « Instinct » : tir instinctif → double flèche.
  ...
  { id: 'sceau-du-heaume', name: 'Sceau du heaume', description: 'Projette un sceau héraldique tournoyant qui percute la cible de plein fouet.', classId: 'chevalier', kind: 'projectile', multiplier: 2.6, cooldownMs: 6000, range: 480, minLevel: 31 },
  ...
  // Passif « double frappe » du chevalier (remplace la régénération du sabreur dans son arbre) :
  // raccourcit le temps de recharge de l'attaque de base selon le rang. Au rang max, il frappe 2× plus souvent.
