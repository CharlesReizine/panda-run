...
|---|---|
| R390 | **quatre cliquets qui avaient cessé de cliqueter** : plafonds réglés 45, 16, 10 et 7 crans au-dessus du réel — ils avaient l'air de protéger et ne touchaient plus rien. Resserrés sur la valeur mesurée. Les libellés de la page Stat sont désormais CALCULÉS depuis les coefficients (« +2 attaque » était recopié à la main), et la chaîne de migration des sauvegardes est testée version par version — la v2 était la seule que rien ne chargeait |
| R389 | **le modèle strict traversait à pied les trous mortels** : `strictReach` — celui qui garde la sélection des graines et qui répond à « ce trampoline est-il la seule solution » — ne découpait la bande de sol qu'aux murs de roche, jamais aux vides. Corrigé, et 2 validateurs qui n'étaient appelés NULLE PART (oiseaux au sol, trampolines facultatifs) enfin branchés sur un test |
| R388 | **le pied du T sous les échelles** (434/593 épaulées) + **toutes les compétences cassent la pierre** (le corps à corps ébréchait une tuile d'un tiers même en ultime, la zone ne touchait rien) + la notif de stats écrite en toutes lettres, même jaune que celle des compétences + **la toile se rapporte au total des points**, plus au maximum |
| R387 | **les recoins où l'on ne tient pas debout deviennent de la pierre** (201 colonnes → 63) : « moins d'un saut d'écart » pris au pied de la lettre aurait touché 2330 colonnes et repeint le jeu — le vrai défaut est la corniche coiffée à UNE rangée, où le panda (qui en fait deux) ne tient pas. Sous filet de sécurité : combler peut MURER, et le premier jet a scellé deux poches de 1492 et 708 cases plus deux monstres |
| R386 | **la page STAT** : quatre stats (INT→VIT renommé, une VRAIE intelligence créée), toile de pondération en %, bouton « Suggérer » par classe, accès depuis le jeu avec pastille contre la barre de vie. « INT » donnait des PV — un libellé qui mentait sur son effet, probablement une des raisons pour lesquelles personne ne répartissait. Migration v8→v9 : l'ancien `int` passe dans `vit`, personne ne perd ce qu'il a investi |
| R385 | **plus une seule échelle qui débouche sur le vide** (49 sur 593) : `laddersToNowhere` EXEMPTAIT les échelles suspendues en supposant qu'une pierre les coiffait — supposition jamais vérifiée, et c'est exactement là que vivaient les « échelles qui volent » signalées depuis des semaines. Règle du joueur : de la terre à hauteur de sortie, ou de la pierre au-dessus, jamais RIEN |
| R384 | **la difficulté re-calibrée sur la deuxième version de la courbe** : « 20 au début, descente douce jusqu'à 10, JAMAIS sous 10 pour un mob normal ». Mesuré nu : 21 → 10, plancher 9,9. Élite à 6 coups (son ATK passe de ×1,1 à ×1,6 — l'ancienne règle « il ne tape pas plus fort » est révisée par le joueur) · **les 10 boss recalibrés** : ils allaient de 3 coups (roi-gloopy) à 1543 (!) — tous à 6 coups désormais, avec 8× la vie d'un mob normal · **un tir emporte une brique d'un coup** (« la mitraillette, c'est pas jouissif du tout ») |
| R383 | **les stats se découvrent enfin** : les boutons « + » n'existaient QUE si des points restaient à dépenser — à zéro, rien ne laissait deviner le système ; et RIEN nulle part (ni HUD, ni carte, ni montée de niveau) ne signalait que des points attendaient. Pastille sur la carte, « + » toujours affiché, et d'où viennent les points · **la tête du monstre dans l'indice de quête** : un nom se lit, une bestiole se reconnaît |
| R382 | **plus un seul trampoline décoratif** : 33 des 35 se contournaient (en retirant l'engin, rien ne devenait injoignable) — la cause était `trampoline-corniche-inverse`, tiré dans le pool GÉNÉRIQUE et posé sur 19 terrains. Atelier plat, corniche et son inverse sortis du catalogue. Reste 10 trampolines, tous sur des obstacles fermés · le TRIPLE rebond devient une variante du mur (10 rangées) plutôt qu'un motif imposé : en motif à part, la gravure n'a jamais convergé — deux heures de calcul, deux échecs |
| R381 | **la courbe de difficulté dictée point par point** : « 14 → 10, 8, 7, 6, 5 et 4 à la fin », personnage NU. Les facteurs d'ATK ont été RÉSOLUS pour chaque niveau, pas devinés. Mesuré : 12,9 · 9,6 · 7,8 · 7,0 · 5,9 · 5,0 · 4,0. C'est l'ATK qui est calibrée (elle seule décide du nombre de coups) ; les PV et la DÉF suivent sa racine — long n'est pas difficile |
| R380 | **les monstres calibrés sur une règle du joueur** : « un mob de ton niveau te tue en 5 coups quand tu as pas de stuff ». Mesuré nu, mob normal de son niveau — niv 5 : 14 coups · niv 10 : 30 · puis 4,5 à 6,6 de 15 à 50. Le début reste un tutoriel, tout ce qui suit tourne autour de cinq coups. ATK, PV ET DÉF durcis (la DÉF sur arbitrage explicite, contre l'avis initial) |
| R379 | **les quatre traversées de rebond enfin dans le jeu** (vide, eau, flammes, fosse de lave) : elles étaient rejetées à la SÉLECTION DE GRAINES, en silence — `oversizedGaps` ignorait qu'un trampoline franchit un gouffre, et le générateur repliait sur un plan sans elles. Huit motifs de rebond posés, aucun motif orphelin sur 105 |
| R378 | **cinq compétences réparées, dont trois qui naissaient DANS LE SOL** : Arcade multiplie la taille du corps par l'échelle du sprite — le lancer d'épée mesurait 64 px de haut lâchés à 16 px sous le centre du panda, soit 8 px sous ses pieds, et le collider sol le détruisait dans la frame même (« ça écrit lancer d'épée mais il se passe rien »). Idem sceau du heaume et boule de feu chargée · **Dévotion ne faisait RIEN depuis toujours** (`applyGuard` jamais appelé) · le mur de flamme se posait au fond de la carte au lieu de la surface visée |
| R377 | **deux motifs où le rebond est la SEULE solution** (mur de pierre de 8 rangées, et sa variante avec trou mortel au pied) · **les 27 trampolines contournables retirés** : mesuré en atteignabilité stricte, retirer l'engin ne rendait RIEN injoignable — trois motifs entiers supprimés du catalogue · **monstres +25 % après le niveau 10, +50 % après 30** (PV et dégâts, par-dessus la pente existante) · rampe de difficulté testée : un obstacle ne peut plus arriver avant l'atelier qui l'enseigne |
| R376 | **la règle du jeu, dite une bonne fois** (panneau d'accueil : tuer des monstres est la SEULE source d'XP — rien ne le disait, et c'est la boucle entière du jeu) · **les oiseaux ne sortent plus de terre** : la convention « sans y = au sol » n'avait jamais été pensée pour ce qui vole, 120 spawns aériens sur 172 démarraient dans le sol · trampolines placés à la meilleure colonne de leur surface (dégagement, corniche desservie, écart aux murs) |
...
| R370 | **la forge : « Réforger » dégagé, « Améliorer » à sa place** — sûr jusqu'à +3, coût lié à la PURETÉ (×1 à ×6), puis 20/25/28/29/30 % de casse qui DÉTRUIT l'objet, bandeau rouge permanent et plan large en cas de succès ou d'échec · **trampolines dégagés** (25 coincés sous une plateforme de terre, glissés sur leur propre support) · le novice porte enfin le bâton · bouton « ← Carte » sur l'écran d'arrivée · témoin de perf et bouton « Grille » retirés · le coffre vide s'annonce « Coffre vide » et se montre 1 fois sur 3 |
