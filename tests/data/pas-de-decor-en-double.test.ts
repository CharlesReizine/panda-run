...
describe('le catalogue de décors ne se répète pas', () => {
  ...
  it('le fichier ne contient plus de zone morte de la taille d\'un catalogue', async () => {
    ...
    // ⚠️ SEUIL RELEVÉ DE 4400 À 4600, ET C'EST DE LA DOCUMENTATION, PAS DU CODE. Ce garde-fou compte les
    // LIGNES pour détecter le recollage d'un catalogue entier (~1000 lignes d'un coup). Les lots de
    // corrections du 4 août ont ajouté plusieurs centaines de lignes de COMMENTAIRE — chaque passe
    ...
    expect(lignes, `${lignes} lignes — un catalogue en double a-t-il été recollé ?`).toBeLessThan(4600)
  })
})
